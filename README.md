...
| **Logistics Status** | Export / local order tracking with a five-step data-entry wizard. An order carries **multiple items** (each with its own quantity, weights, IDM, and — for exports — its own export number, since one order can be filed under several exports) and **multiple containers**. List, wizard, and detail are all real screens now. Logistics orders don't carry requisition/required dates or a delay figure — that upstream-of-shipping tracking belongs to Imports Status, not here. Below its own orders table, the list also surfaces a read-only **Open Requests — Import FOB** section: FOB imports past Under Production need Logistics to arrange sea freight and a clearing agent, so those consignments appear here live (never copied) with a link back to the real record in Imports Status. |
| **Trucking Status** | Vehicle-movement jobs (Intrafactory / Outbound / Inbound) with a four-step data-entry wizard covering movement details, per-vehicle tracking, freight & payment, and a delivered-fraction rollup. Its list also surfaces live, never-copied "open request" rows read straight from Logistics Status and Imports Status once they're past production, tagged by source — **Logistics** (outbound legs ready to move), **Import FOB** (inbound legs for FOB imports), and **Export** (export orders that are packed/ready and need trucking to the port before they can sail) — filterable via the Source dropdown. See [Data & the backend](#data--the-backend). |
