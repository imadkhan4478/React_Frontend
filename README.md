...
| **Imports Status** | Consignment tracking with a **six-step** data-entry wizard (Consignment, Finance, Shipping, Payments, Status & Remarks, Clearance — the old Landed Cost step was removed since accounts, not this system, finalise ELC/ALC). Step 1 carries an **Incoterm** field (FOB / CIF / CFR / EXW / DAP) alongside requisition/required dates with a live delay readout — a consignment marked FOB and past Under Production is what both Logistics Status's and Trucking Status's live request feeds pick up. The detail view's final section is a **Total expenditure** summary (goods value + bank charges + demurrage, pulled from figures already captured in Finance/Payments/Clearance) rather than a per-item landed-cost table. The list shows a **Reference** column (the LC/DP instrument number) and the search box matches against it directly; there's no separate payment-instrument filter dropdown. |
| **Logistics Status** | Export / local order tracking with a **five-step** data-entry wizard: Order Details, **Packing** (replaces the old Transportation step), Shipping, Expenditures, Status. Step 1 carries **MO No.** and **Incoterm** alongside the item lines, each item now also tracking **Job #**, a real **Unit Weight** input, a derived read-only **Net Weight** (quantity × unit weight), and **Planned/Actual RFD dates** — every RFD date change is logged to a per-item audit trail (who, when, old → new value), surfaced as system-generated entries in the Status step's remarks feed. Packing replaces Transportation entirely: one or more **packages**, each with its own cost/status/weight and a **per-item quantity allocation** (a package routinely holds partial quantities of more than one item), with a combined **outstanding-quantity** summary across all packages. Status adds a derived **Marketing Delay** (packing date vs. gate-out date, or today), the merged remarks feed (user entries + RFD-change system entries, editable only by an admin), and a one-way **Send to Trucking** handoff — checking it and saving turns the order into an open request in Trucking Status, with a live read-through panel here once a Trucking job exists against it. Logistics no longer tracks requisition/required dates, transporter, or a delivery destination — that's now entirely Trucking's job once handed off. Below its own orders table, the list also surfaces a read-only **Open Requests — Import FOB** section: FOB imports past Under Production need Logistics to arrange sea freight and a clearing agent, so those consignments appear here live (never copied) with a link back to the real record in Imports Status. |
| **Trucking Status** | Vehicle-movement jobs (Intrafactory / Outbound / Inbound) with a four-step data-entry wizard covering movement details, per-vehicle tracking, freight & payment, and a delivered-fraction rollup. Its list surfaces live, never-copied "open request" rows read straight from Logistics Status (once an order's **Send to Trucking** box is checked) and Imports Status (FOB imports past Under Production), tagged by source — **Logistics**, **Import FOB**, and **Export** (export orders packed/ready for the port leg, which stays always-live with no accept step) — filterable via the Source dropdown, plus a separate **Pending requests only** filter for just the still-open, not-yet-taken derived rows (distinct from "Open requests only", which also covers in-progress manual jobs). A **Take Action** button on any Logistics or Import-FOB request converts it into an independent job: it snapshots the source's items (or, for a Logistics order with packages, its packages) into the new job, opens the wizard pre-filled, and the request permanently stops being re-derived from that point on — a taken request never shows twice. If the source order had packages, the Vehicles step gains a **per-vehicle package checklist** so the operator can assign which package rides on which truck (not required to be 1:1 — assigning the same package to two vehicles warns rather than blocks). See [Data & the backend](#data--the-backend). |
